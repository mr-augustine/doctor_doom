...
#define MISSION_TIMEOUT         400

Statevars myVars;
Compass myCompass(&myVars);
Control myControl(&myVars);
Gps myGps(&myVars);
LedButton myButton;
Logger myLogger(&myVars);
Mobility myMobility(&myVars);
Odometer myOdometer(&myVars);
Navigation myNavigation(&myVars, &myCompass, &myGps, &myOdometer, (float) (MAIN_LOOP_TIMER_PRESCALER / CLOCK_SPEED_HZ));
Robot doom(&myVars, &myCompass, &myControl, &myGps, &myButton, &myOdometer, &myNavigation);
